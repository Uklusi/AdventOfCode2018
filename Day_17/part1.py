...
def printImage():
    ...
    imageList = []
    for y in range(ymin, ymin + viewVert):
        row = []
        for x in range(500 - view, 500 + view + 1):
            p = Position(x,y)
            if p in clay:
                row.append(solid)
            elif p in water:
...
result = len( {p for p in wet if ymin <= p.y <= ymax} | {p for p in water if ymin <= p.y <= ymax} )
